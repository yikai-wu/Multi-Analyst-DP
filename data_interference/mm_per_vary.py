...
print('n={}, k_max={}, eps={}, rep={}, seed={}, t={}, tol={}, sample={}, x_file={}'.format(n, k_max,eps,rep,seed,t,tol,sample, x_file))
conf = OrderedDict()
conf['n']=n
conf['k_max']=k_max
conf['sample']=sample
...
W_name = np.array(['Mean', 'Median', 'Per_25', 'Per_75'])
W_lst = np.array([meanW, workload.Prefix(n), workload.Prefix(n), workload.Prefix(n)])
A_lst = strategy_comp(W_lst, n, rep)
q_lst = W_name

ans_lst = np.array([x_mean, x_median] + x_per)
print(ans_lst)
...
for i in range(t):
    ...
    Wn = W_name[ls]
    names.append(Wn)
    error_base, total_error, max_ratio_error, inter = interference_data(W_lst, A_lst, ls, n, eps, modes, rep, qk=q_lst, x=x_data, ansk=ans_lst, sample=sample, tol=tol)
...
file_name = experiment_name + '_k{}_t{}_eps{}_sam{}_tol{}_run{}'.format(k_max, t, eps, sample, tol, run)
